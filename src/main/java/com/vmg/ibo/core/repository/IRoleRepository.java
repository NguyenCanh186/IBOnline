...
public interface IRoleRepository extends JpaRepository<Role, Long> {
    ...
    @Query(value = "SELECT  r FROM Role r where (:name is null OR lower(r.name) LIKE  %:name% or lower(r.code) LIKE %:name%) AND r.code != 'KH'")
    Page<Role> findAllByNameContaining(String name, Pageable pageable);

    @Query("SELECT r FROM Role r WHERE r.status = 1 AND r.code != 'KH'")
